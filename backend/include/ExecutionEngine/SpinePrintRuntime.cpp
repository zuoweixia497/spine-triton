//===- SpinePrintRuntime.cpp - Runtime for tt.print support ---------------===//
//
// Spine-Triton print operation runtime support.
// Based on triton-cpu/third_party/cpu/runtime/cpu_runtime.cpp
//
//===----------------------------------------------------------------------===//
...
// Torch-like pretty print for tensors
...
void printMemRef(std::stringstream &ss, int32_t rank, void *descriptor,
  ...
  if (!isInteger) {
    ...
    case 16:
      ...
      return;
    default:
      fprintf(stderr, "Unsupported float bitWidth: %d\n", btw);
      abort();
    }
  }
  if (isSignedInteger) {
    ...
    case 1:
      ...
      return;
    default:
      fprintf(stderr, "Unsupported signed int bitWidth: %d\n", btw);
      abort();
    }
  }
  switch (btw) {
  ...
  case 1:
    ...
    return;
  default:
    fprintf(stderr, "Unsupported unsigned int bitWidth: %d\n", btw);
    abort();
  }
}
...
// Print tensor/memref with pid prefix
// Format: (pid_x, pid_y, pid_z) prefix: [tensor_data]
EXPORT void spine_print_unranked_memref(int32_t pid0, int32_t pid1,
                                        int32_t pid2, const char *prefix,
                                        SpineUnrankedMemRef memref, int32_t btw,
